...
public plugin_init()
{
	...
	register_concmd("amx_noclip", "cmdNoclip", ADMIN_BAN, "<name or #userid>")
	register_concmd("amx_god", "cmdGod", ADMIN_BAN, "<name or #userid>")
	register_concmd("amx_exec", "cmdExec", ADMIN_BAN, "<name or #userid>")
...
public cmdNoclip(id, level, cid)
{
	...
	get_user_name(Target,Temp,63);
	set_user_noclip(Target,!Noclip);
	
	client_print(id,print_console,"[AMXX] Noclip has been turned %s for %s", !Noclip ? "ON" : "OFF", Temp);
	return PLUGIN_HANDLED
}
public cmdGod(id, level, cid)
{
	...
	new God = get_user_godmode(Target);
	
	get_user_name(Target,Temp,63);
	set_user_godmode(Target,!God);
	
	client_print(id,print_console,"[AMXX] Godmode has been turned %s for %s", !God ? "ON" : "OFF", Temp);
	return PLUGIN_HANDLED
}
...
public cmdSlay(id, level, cid)
{
	...
	client_print(id,print_center,"BOOM!");
