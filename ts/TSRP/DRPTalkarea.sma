...
// Sounds
new const g_SmsSound[] = "OZDRP/phone/sms.wav"
new const g_RingSound[] = "OZDRP/phone/ring.wav"
new const g_VibrateSound[] = "OZDRP/phone/drpvibrate.wav"
new const g_ScanSound[] = "items/suitcharge1.wav"
new const g_NotFoundSound[] = "items/suitchargeno1.wav"
new const g_FoundSound[] = "items/suitchargeok1.wav"

new const g_OOCSound[] = "sound/OZDRP/arp_ooc.wav"
new const g_AdSound[] = "sound/buttons/blip2.wav" // half-life

// Models
new const g_szVPhoneMdl[] = "models/OZDRP/phone/v_cell_new.mdl"
new const g_szPPhoneMdl[] = "models/OZDRP/phone/p_cell_new.mdl"

// Menus
new const g_MainPhoneMenu[] = "DRP_MainPhoneMenu"
new const g_DialPhoneMenu[] = "DRP_DialNumberMenu"
new const g_FeatureMenu[] = "DRP_FeatureMenu"
...
public DRP_Init()
{
	...
	// CVars
	p_Refresh = register_cvar("DRP_OOCRefresh","10");
	...
	hook_cvar_change(p_Refresh, "_OOCRefresh");
...
public _OOCRefresh(pcvar, const old_value[], const new_value[])
{
}
...
DialNumber(id,Func)
{
	...
	Pos += formatex(Menu[Pos],255 - Pos,"Enter Number (%s)^n^n",Func ? "Calling" : "Registering");
	Pos += formatex(Menu[Pos],255 - Pos,"----------------------^n");
	
	Pos += formatex(Menu[Pos],255 - Pos,"       %d%d%d-%d%d%d        ^n",
	g_UserNumber[id][0],g_UserNumber[id][1],g_UserNumber[id][2],g_UserNumber[id][3],g_UserNumber[id][4],g_UserNumber[id][5]);
	
	Pos += formatex(Menu[Pos],255 - Pos,"----------------------^n^n");
	Pos += formatex(Menu[Pos],255 - Pos,"1. One^n2. Two^n3. Three^n4. Four^n5. Five^n6. Six^n7. Seven^n8. Eight^n9. Nine^n^n0. Exit");
	
	g_UserFunc[id] = Func
	
	show_menu(id,g_Keys,Menu,-1,g_DialPhoneMenu);
}
public DPhone(id)
{
	if(g_UserPhone[id][MODE] == MODE_DEACTIVATING)
