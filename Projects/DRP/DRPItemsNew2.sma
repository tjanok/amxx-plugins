...
// Sounds
new const g_Lockpick[] = "OZDRP/lockpick.wav"
...
public DRP_RegisterItems()
{
	...
	DRP_RegisterItem("Djarum Black Cigarette","_Smoke","Best Flavored Cigarette.",1,_,_,65,6);
	...
	DRP_RegisterItem("Viva La Drak","_Alcohol","A bottle of some extremly expensive wine.",1,_,_,80);
	DRP_RegisterItem("Spanky's XXX","_Alcohol","A bottle of some extremly expensive wine.",1,_,_,80);
...
public DRP_HudDisplay(id,Hud)
{
	...
	if(g_Eating[id])
		DRP_AddHudItem(id,HUD_SEC,"Action: Eating/Drinking");
	
	if(g_Alcohol[id])
		DRP_AddHudItem(id,HUD_SEC,"Action: Alcohol Level: %d^%",g_Alcohol[id]);
}
...
public _Food(id,ItemID,Eating,HungerLoss,HPGain)
{
	...
	else if(DRP_GetUserHunger(id) < HungerLoss)
	{
...
public DrinkAlcohol(const id)
{
	if(g_Alcohol[id] <= 0 || !is_user_alive(id))
		return set_user_rendering(id);
	
	// The more drunk they are
	// the darker is the screen is going to be
	
	if(g_Alcohol[id] >= 100)
		UTIL_ScreenFade(id,20,15,0,0,0,255);
	else if(g_Alcohol[id] >= 50)
		UTIL_ScreenFade(id,15,3,random_num(10,255),random_num(20,255),random_num(80,255),g_Alcohol[id] >= 80 ? 225 : 165);
	...
	g_Alcohol[id] -= 5
	return set_task(15.0,"DrinkAlcohol",id);
}
/*==================================================================================================================================================*/
...
public _Smoke(id,ItemID,Time,HPLose)
{
	if(g_Smoking[id][TIMELEFT] > 0 && !g_Smoking[id][IN_PROGRESS])
	{
		...
		DRP_SetUserItemNum(id,g_Smoking[id][ITEMID],DRP_GetUserItemNum(id,g_Smoking[id][ITEMID]) + 1);
	}
...
public _Tazer(id,ItemID)
{
	...
	*/
	...
	if(g_Tazered[id])
	{
		client_print(id,print_chat,"[DRP] You cannot tazer someone else while you are tazered.");
		return PLUGIN_HANDLED
	}
	...
	new Float:pOrigin[3],Float:tOrigin[3]
	...
	engfunc(EngFunc_MessageBegin,MSG_PVS,SVC_TEMPENTITY,tOrigin,0);
	...
	client_print(Index,print_chat,"[DRP] You have been tasered.");
	...
	g_MaxSpeed[Index] = pev(Index,pev_maxspeed);
...
public forward_PreThink(const id)
{
	if(g_Eating[id] || g_Tazered[id])
	{
		...
		set_pev(id,pev_maxspeed,g_Tazered[id] ? 0.0 : g_MaxSpeed[id] / 2);
	}
}
...
{
	new Ent = 0
	while(( Ent = engfunc(EngFunc_FindEntityByString,-1,"classname",g_Lightbulb)) != 0)
		if(pev(Ent,pev_owner) == id)
			engfunc(EngFunc_RemoveEntity,Ent);
