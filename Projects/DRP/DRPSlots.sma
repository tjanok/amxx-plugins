...
public plugin_init()
{
	register_plugin("DRP Slots Reservation", AMXX_VERSION_STR, "AMXX Dev Team / Drak");
	...
	p_CVar = get_cvar_pointer("sv_visiblemaxplayers");
}
...
public client_authorized(id)
{
	...
	// There not an admin (with the flag ADMIN_RESERVATION)
	// And the player limit hasn't been reached
	// Check if we are a DRP Admin (it has to be delayed)
	
	set_task(2.0,"CheckDRPAdmin",id);
	return PLUGIN_HANDLED
}

public CheckDRPAdmin(const id)
{
