...
public plugin_precache()
{
	...
	{
		formatex(Cvar,32,"DRP_HUD%d_X",Count + 1);
		p_Hud[Count][X] = register_cvar(Cvar,"");
		
		formatex(Cvar,32,"DRP_HUD%d_Y",Count + 1);
		p_Hud[Count][Y] = register_cvar(Cvar,"");
		
		formatex(Cvar,32,"DRP_HUD%d_R",Count + 1);
		p_Hud[Count][R] = register_cvar(Cvar,"");
		
		formatex(Cvar,32,"DRP_HUD%d_G",Count + 1);
		p_Hud[Count][G] = register_cvar(Cvar,"");
		
		formatex(Cvar,32,"DRP_HUD%d_B",Count + 1);
	...
	g_ClassForward = CreateMultiForward("DRP_ClassLoaded",ET_IGNORE,FP_CELL,FP_STRING,FP_CELL);
...
public plugin_init()
{
	...
	// Entity 'Godding'
	new Temp
	Temp = get_cvar_num("DRP_GodDoors");
	if(Temp)
	{
		new Ent
		if(Temp == 2)
	...
	Temp = get_cvar_num("DRP_GodBreakables");
	if(Temp)
	{
		new Ent
		if(Temp == 2)
...
SQLInit()
{
	...
	format(g_Query,4095,"CREATE TABLE IF NOT EXISTS %s (Internalname VARCHAR(66),Externalname VARCHAR(66),Ownername VARCHAR(40),OwnerAuthID VARCHAR(36),Price INT(11),Access VARCHAR(27),Profit INT(11),CustomMessage TEXT,PRIMARY KEY (Internalname))",g_PropertyTable)
	UTIL_CleverQuery(g_Plugin,g_SqlHandle,"IgnoreHandle",g_Query)
	...
	format(g_Query,4095,"CREATE TABLE IF NOT EXISTS `ItemDrops` (ItemName VARCHAR(33),Num INT(11),X FLOAT(11),Y FLOAT(11),Z FLOAT(11),SQLKey INT(11),PRIMARY KEY(SQLKey))");
	UTIL_CleverQuery(g_Plugin,g_SqlHandle,"IgnoreHandle",g_Query);
	
	format(g_Query,4095,"CREATE TABLE IF NOT EXISTS %s (ClassKey VARCHAR(64),Value TEXT,PRIMARY KEY (ClassKey))",g_DataTable);
	UTIL_CleverQuery(g_Plugin,g_SqlHandle,"IgnoreHandle",g_Query);

	format(g_Query,4095,"CREATE TABLE IF NOT EXISTS %s (SteamID VARCHAR(36),BankMoney INT(11),WalletMoney INT(11),JobName VARCHAR(33),Hunger INT(11),Access VARCHAR(24),JobRight VARCHAR(24),PlayTime INT(11),PRIMARY KEY (SteamID))",g_UserTable);
	UTIL_CleverQuery(g_Plugin,g_SqlHandle,"IgnoreHandle",g_Query);
...
public FetchProperty(FailState,Handle:Query,Error[],Errcode,Data[],DataSize)
{
	...
	new InternalName[127],ExternalName[64],OwnerName[33],OwnerAuthid[36],Price,AccessStr[JOB_ACCESSES + 1],Access,Profit,CurArray
	while(SQL_MoreResults(Query))
	{
...
public FetchDoors(FailState,Handle:Query,Error[],Errcode,Data[],DataSize)
{
	...
	{		
		...
		array_set_int(CurArray,4,SQL_ReadResult(Query,2)); // Override door lock
...
public CmdBuy(id)
{
	...
	Data[0] = id
	Data[1] = Property + 1
	Data[2] = (Price > g_UserBank[id]) ? 0 : 1
	...
	new ExternalName[33]
	array_get_string(CurArray,1,ExternalName,32);
	
	if(OtherAuthID[0] && Price)
	{
		...
		for(new Count;Count < iNum;Count++)
		{
			...
			if(equali(PlayerAuthid,OtherAuthID))
			{
		...
		if(!Flag)
		{
			format(g_Query,4095,"UPDATE %s SET bankmoney = bankmoney + %d WHERE SteamID='%s'",g_UserTable,Price,OtherAuthID);
			UTIL_CleverQuery(g_Plugin,g_SqlHandle,"IgnoreHandle",g_Query);
		}
	}
...
public ItemsHandle(id,Menu,Item)
{
	...
	else
	{
		...
		format(g_Menu,255,"Item: %s ( x %d )^n^n1. Use^n2. Give^n3. Drop^n4. Show^n5. Examine^n^n6. Back^n^n0. Exit",ItemName,UTIL_GetUserItemNum(id,ItemID));
...
{
	if(!get_pcvar_num(p_FLName) || is_user_admin(id) || !equal(key,"name"))
		return FMRES_IGNORED
	...
	set_user_info(id,key,"John Doe");
	client_print(id,print_chat,"[DRP] You name has been changed. Please use a first and last name.");
	return FMRES_SUPERCEDE
}
...
public FetchClientData(FailState,Handle:Query,Error[],Errcode,Data[],DataSize) 
{
	...
	if(SQL_NumResults(Query) < 1)
	{
		...
		DRP_CallEvent("DRP_NewPlayer",Data,1);
...
public FetchClientKeys(FailState,Handle:Query,Error[],Errcode,Data[],DataSize) 
{
	...
	new InternalName[33],Property,AuthidKey[64],Garbage[1],CurArray
	g_GotInfo[id]++
	...
	while(SQL_MoreResults(Query))
	{
		...
		if(Property != -1)
		{
			CurArray = array_get_int(g_PropertyArray,Property);
...
public forward_PreThink(id)
{
	...
	new Index,Body
	get_user_aiming(id,Index,Body,90);
	...
	if(find_sphere_class(id,g_szNPCName,100.0,EntList,1))
	{
	...
	if(find_sphere_class(id,g_szItem,40.0,EntList,1))
	{
		new Ent = EntList[0]
		...
		if(Key)
		{
			format(g_Query,4095,"DELETE FROM `ItemDrops` WHERE SQLKey=%d",Key);
			SQL_ThreadQuery(g_SqlHandle,"IgnoreHandle",g_Query);
		}
	...
	if(equali(Classname,g_FuncDoor) || equali(Classname,"func_door_rotating"))
	{
		new Door
		pev(Index,pev_targetname,Classname,63);
		
		new const Property = UTIL_GetProperty(Classname,Index,Door);
		if(Property == -1)
			return FMRES_IGNORED
		server_print("Door: %d-%d",Door,array_get_int(array_get_int(g_DoorArray,Door),4))
		...
		if(equali(Classname,AuthID) || array_get_int(CurArray,8) & (1<<(id - 1)) || array_get_int(CurArray,6) & g_UserAccess[id])
		{
...
PrintDisplay(id,Index)
{
	static Classname[64],Message[128],EntList[1]
	EntList[0] = 0
	...
	if(find_sphere_class(id,g_szNPCName,100.0,EntList,1))
	{
		...
		// Noise1 = NPC Name (Don't use the stock here)
		pev(Ent,pev_noise1,Classname,63);
		formatex(Message,127,"NPC: %s^nPress use (default e) to use",Classname);
		
		// Client_Print(CENTER) Will always be used for property/npcs
	...
	else if(containi(Classname,g_FuncDoor) != -1)
	{
		...
		new const CurArray = array_get_int(g_PropertyArray,Property);
		...
		formatex(Message,127,"%s^nOwner: %s ( %s )^nStatus: %s^n%s^n%s",Name[0] ? Name : "",Classname[0] ? Classname : "None",Price ? "Selling" : "Owned",array_get_int(array_get_int(g_DoorArray,Door),5) ? "Locked" : "Unlocked",Price ? Temp : "",CMessage[0] ? CMessage : "");
		client_print(id,print_center,"^n^n^n^n^n^n^n^n^n^n^n^n^n^n^n^n%s",Message)
	}
	if(!g_HudPending)
		{ g_Display[id] = 0; set_task(1.6,"ResetDisplay",id); }
}
...
public _DRP_GetUserTime(Plugin,Params)
{
	...
	new Total = ((get_user_time(id) + g_UserTime[id]) / 60)
	return Total
}
/*==================================================================================================================================================*/
...
public _DRP_AddProperty(Plugin,Params)
{
	...
	new InternalName[64],ExternalName[64],OwnerName[33],OwnerAuth[36],Price,Access,Profit,AccessStr[JOB_ACCESSES + 1]
	...
	format(g_Query,4095,"INSERT INTO %s VALUES ('%s','%s','%s','%s','%d','%s','%d','')",g_PropertyTable,InternalName,ExternalName,OwnerName,OwnerAuth,Price,AccessStr,Profit);
	SQL_ThreadQuery(g_SqlHandle,"IgnoreHandle",g_Query);
...
public _DRP_AddDoor(Plugin,Params)
{
	...
	array_set_string(CurArray,0,Targetname);
	EntID ? array_set_int(CurArray,1,EntID) : 0
	array_set_string(CurArray,2,InternalName);
	array_set_int(CurArray,3,1);
	array_set_int(CurArray,4,get_param(4));
...
public IgnoreHandle(FailState,Handle:Query,const Error[],Errcode,const Data[],DataSize) 
{
	...
	server_print("[IGNOREHANDLE] Query");
...
{
	new Data[1]
	if(_CallEvent("Core_Save",Data,1))
		return
	...
	{		
		...
		format(g_Query,4095,"INSERT INTO %s VALUES ('%s','%s','%s','%s','%d','%s','%d','%s') ON DUPLICATE KEY UPDATE externalname='%s',ownername='%s',ownerauthid='%s',price='%d',access='%s',profit='%d',custommessage='%s'",g_PropertyTable,
		InternalName,ExternalName,OwnerName,OwnerAuthid,Price,AccessStr,Profit,Message,
		ExternalName,OwnerName,OwnerAuthid,Price,AccessStr,Profit,Message);
	...
	for(new Count;Count < g_DoorNum;Count++)
	{
		...
		Locked = array_get_int(CurArray,3);
		
		EntID ? 
			format(Targetname,32,"e|%d",EntID) : format(Targetname,32,"t|%s",Targetname);
		
		format(g_Query,4095,"INSERT INTO %s VALUES ('%s','%s','%d') ON DUPLICATE KEY UPDATE internalname='%s'",g_DoorsTable,Targetname,InternalName,Locked,InternalName);
		UTIL_CleverQuery(g_Plugin,g_SqlHandle,"IgnoreHandle",g_Query);
